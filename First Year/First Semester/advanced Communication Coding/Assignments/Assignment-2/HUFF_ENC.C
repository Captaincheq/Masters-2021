...
void main(int argc, char **argv)
{
	...
	unsigned char *compressed_file;											/*pointer to an array for file */
	...
	int values[256], loc[256];	//value is the frequency of the symbols
	...
	if(t == 0)		//no input file, create a temporary file for input
	{
		strcpy(infile,"tmpf");
	...
	if(cfp == NULL)
	{
		...
		/* set to zero, codebook of the symbols */
		for(i=0; i<num; i++)
		{
			code[i]=0;
			length[i]=0;
	...
	/* encode file */
	/* get memory for compressed_file */
    compressed_file = (unsigned char*) malloc (size * 2 * sizeof(unsigned char) );
    if (compressed_file == NULL)
	{
		printf("Unable to allocate memory for file.\n");
        exit(1);
	}
	size = files(size, code, length, file, compressed_file);
	...
	*/
    ...
    fwrite(compressed_file,sizeof(unsigned char),size,ofp);
    fclose(ofp);
